...
def create_persistent_attribute(node, module_node, *args, **kwargs):
    ...
    if cmds.attributeQuery(module_attr_name, node=module_node, exists=True):
        ...
        data_type = cmds.addAttr(
            module_node + "." + module_attr_name, query=True, dataType=True
        )
        kwargs = {}
    ...
    cmds.connectAttr(node + "." + long_name, module_node + "." + module_attr_name)
...
def get_attributes_state(node):
    ...
    for attr in cmds.listAttr(node):
        ...
        attr_name = node + "." + attr
        attr_state = {}
        attr_state["lock"] = cmds.getAttr(attr_name, lock=True)
        attr_state["keyable"] = cmds.getAttr(attr_name, keyable=True)
        attr_state["channelBox"] = cmds.getAttr(attr_name, channelBox=True)
