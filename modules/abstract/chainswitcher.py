...
from mop.core.fields import IntField, ObjectListField, ObjectField, StringField
import mop.metadata
...
class ChainSwitcher(Chain):
    ...
    def _create_chains(self):
        self.chain_a.set(
            cmds.duplicate(self.deform_joints, renameChildren=True, parentOnly=True)
        )
    ...
    def _create_settings_control(self, node=None):
        ...
        ctl, buffer_grp = self.add_control(
            node, description='settings', shape_type='cogwheel'
        )
        ...
        for attr in ['translate', 'rotate', 'scale']:
            for axis in 'XYZ':
                ...
                cmds.setAttr(attrName, lock=True, keyable=False, channelBox=False)
        cmds.setAttr(ctl + '.visibility', lock=True, keyable=False, channelBox=False)
    ...
    def _setup_switch(self):
        ...
        self.reverse_switch.set(self.add_node('reverse', description='switch'))
        cmds.connectAttr(
        ...
        for i, deform in enumerate(self.deform_joints):
            ...
            cmds.connectAttr(
                a + ".worldMatrix[0]", wt_add_mat + ".wtMatrix[0].matrixIn"
            )
            cmds.connectAttr(
                b + ".worldMatrix[0]", wt_add_mat + ".wtMatrix[1].matrixIn"
            )
            cmds.connectAttr(
            ...
            cmds.connectAttr(wt_add_mat + ".matrixSum", mult_mat + '.matrixIn[0]')
            cmds.connectAttr(
                deform + '.parentInverseMatrix[0]', mult_mat + '.matrixIn[1]'
            )
            ...
            cmds.connectAttr(deform + '.jointOrient', euler_to_quat + '.inputRotate')
            ...
            cmds.connectAttr(decompose_mat + '.outputTranslate', deform + '.translate')
            cmds.connectAttr(quat_to_euler + '.outputRotate', deform + '.rotate')
            cmds.connectAttr(decompose_mat + '.outputScale', deform + '.scale')
